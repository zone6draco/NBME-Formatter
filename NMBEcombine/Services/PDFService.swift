...
enum PDFService {
    ...
    static func buildFinalReport(
        ...
        outputURL: URL,
        log: (String) -> Void,
        onProcessProgress: @escaping (Int, Int) -> Void,
        onSaveProgress: @escaping (Int) -> Void
    ) throws {
        ...
        var test1=1
        for item in ordered {
            onProcessProgress(test1,ordered.count)
            test1+=1
            let q = item.question
        ...
        var completed = 0
        var currentIndex = 0
        log("Merging PDFs, this might take a minute...")
